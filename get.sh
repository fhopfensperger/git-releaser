...
verifySupported() {
  ...
  if ! echo "${supported}" | grep -q "${OS}-${ARCH}"; then
    ...
    echo "To build from source, go to https://github.com/fhopfensperger/git-releaser"
    exit 1
  fi
...
checkDesiredVersion() {
  if [ "x$DESIRED_VERSION" == "x" ]; then
    ...
    TAG=$(curl --silent "https://github.com/fhopfensperger/git-releaser/releases/latest" | sed 's#.*tag/\(.*\)\".*#\1#')
  else
    TAG=$DESIRED_VERSION
  fi
}

# checkInstalledVersion checks which version of git-releaser is installed and
...
downloadAndInstall() {
  ...
  downloadUrl=$(curl -s https://api.github.com/repos/fhopfensperger/git-releaser/releases/latest | grep browser_download_url | grep ${OS}_${ARCH} | cut -d ":" -f 2,3 | cut -d " " -f 2,3 | tr -d \")
